...
import (
	...
	"errors"
	"fmt"
	yapi "megin/library/apidoc/yapiclient"
...
func NewYapi(config *YapiConfig) (*yapi.Client, error) {
	yapiClient, err := yapi.NewClient(config.Url, config.Token)
	yapiClient.CateId = config.CateId
	if err != nil {
		panic(err)
	}
...
func SaveApi(doc *astparser.ApiDoc, config *YapiConfig) error {
	yapiClient, err := NewYapi(config)
	if err != nil {
		return err
	}

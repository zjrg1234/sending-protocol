...
func SuccessMessage(data any, message string) *Result {
	return &Result{
		Code:    STATUS_SUCESS,
		Message: message,
		Success: true,
...
func Failed(err error, msgarg ...string) *Result {
	tips := ""
	if len(msgarg) > 0 {
		tips = msgarg[0]
	}

	if bizErr, ok := errs.IsBusinessError(err); ok {
		if len(tips) > 0 {
			bizErr.Message = tips
		}
	...
	if normalErr, ok := errs.IsNormalError(err); ok {
		if len(tips) > 0 {
		...
		return &Result{
			...
			Detail:  fmt.Sprintf("%+v", normalErr.Err),
			Success: false,
		}
	}

...
import (
	...
	"go.uber.org/zap"
	"math/rand"
	"megin/library/apidoc"
	"megin/library/astparser"
	"megin/library/context/result"
	"megin/library/logger"
	"megin/library/rpc/protocol"
	"megin/library/strings"
	"megin/system/config"
	"megin/system/jwtclaim"
)
...
type Context struct {
	...
	Log          logger.Log
	JwtClaimData jwtclaim.JwtClaimData
	error        error
...
func (ctx *Context) Param(key string) string {
	if ctx.IsRpcRequest {
		if id, ok := ctx.RpcArgs.Headers[key]; ok {
			return id
		}

		if data, ok := ctx.RpcArgs.Data.(map[string]any); ok {
...
func NewContext(ginCtx *gin.Context) *Context {
	...
	//解析token中的附加信息
	jwtClaimData, exist := ginCtx.Get(jwtclaim.JwtClaimDataKey)
	if exist {
		if jwtData, ok := jwtClaimData.(jwtclaim.JwtClaimData); ok {
...
func GenDoc(ctx *Context, fn ResultHandleFunc, result *result.Result) {
	...
	conf := config.GetConfig()
	apidoc.SaveApi(&ctx.ApiDoc, &apidoc.YapiConfig{
		Url:    conf.Yapi.Url,
		Token:  conf.Yapi.Token,
		CateId: conf.Yapi.CateId,
	})
}
